...
struct Grupo {
	...
	int numJugado, aportacion, premio;	
};

//VARIABLES GLOBALES PARA LOS BUCLES Y LA OPCIÓN DEL MENÚ
int i, j, opcion;
...
//FUNCIONES DE LA AMPLIACIÓN
int menu (void);
void gestionaGrupo (struct Grupo amigos[]);
...
int main (void) {
	...
	struct Grupo amigos[MAXAMIGOS];
	int bomboNumeros[TOTALPREMIOS], bomboPremios[PREMIOSMAYORES+PREMIOSMENORES], cantidadAmigos;	
	//REPRODUCIR SONIDO
	...
	do {
		menu();
		switch (opcion) {
			//COMPROBAR UNO O VARIOS NÚMEROS
			case 1:
				do {
			...
			break;
			//MENU DE LA COLLA DE AMIGOS
			case 2: 
				gestionaGrupo(amigos);				
				//PREGUNTAMOS CUANTOS AMIGOS QUIEREN JUGAR
				do{
					printf("¿Cuantos amigos sois en total?\n"); scanf ("%d", &cantidadAmigos);
					if (cantidadAmigos <2 || cantidadAmigos>MAXAMIGOS) printf ("Esta cantidad no se puede calcular\n");
				}while(cantidadAmigos<2 || cantidadAmigos>MAXAMIGOS);  
				printf("Nombre			Numero		Aportacion\n");
				for (i = 0; i < cantidadAmigos; i++) {		
					printf("%s			%05d			%d\n", amigos[i].nombre, amigos[i].numJugado, amigos[i].aportacion);
				}
							
				//inicializaPremioGanado(&premioGanado);
				for (i = 0; i < cantidadAmigos; i++) {
					inicializaPremioGanado(&premioGanado);
					premioGanado.numConsulta = amigos[i].numJugado;
					premioGanado.cantidad = amigos[i].aportacion;
					if (compruebaPremiosMayores (premiosMayores, &premioGanado)) {
						for (i = 0; i < cantidadAmigos; i++) {
							if (premioGanado.numConsulta == premiosMayores[0].numero) 
								amigos[i].premio = PRIMER_PREMIO /PREMIOSERIE * premioGanado.cantidad;			
							else if (premioGanado.numConsulta == premiosMayores[1].numero) 
								amigos[i].premio = SEGUNDO_PREMIO /PREMIOSERIE * premioGanado.cantidad;			
							else if (premioGanado.numConsulta == premiosMayores[2].numero)
								amigos[i].premio = TERCER_PREMIO /PREMIOSERIE * premioGanado.cantidad;			
							else if (premioGanado.numConsulta == premiosMayores[3].numero || premioGanado.numConsulta == premiosMayores[4].numero)
								amigos[i].premio = CUARTO_PREMIO /PREMIOSERIE * premioGanado.cantidad;	
							else {
								for (i = 5; i < PREMIOSMAYORES; i++) {
									if (premioGanado.numConsulta == premiosMayores[i].numero) 
										amigos[i].premio = QUINTO_PREMIO /PREMIOSERIE * premioGanado.cantidad;						
								}		
							}
					}
					}
						...
						if (compruebaPremiosMenores(premiosMenores, &premioGanado))							
								for (i = 0; i < cantidadAmigos; i++) {
									amigos[i].premio = premioGanado.premioMenor;	
								}
							}
					if (!compruebaPremiosMayores(premiosMayores, &premioGanado) && !compruebaPremiosMenores(premiosMenores, &premioGanado))
							if (compruebaPremioConsolacion(premiosMayores, &premioGanado))
								for (i = 0; i < cantidadAmigos; i++) {
									if(premioGanado.premioReintegro1o !=0) amigos[i].premio = premioGanado.premioReintegro1o;
								}		
					}	
						
				for (i = 0; i < cantidadAmigos; i++) {
					printf("Numero: %05d Aportacion: %d Premio: %d\n", amigos[i].numJugado, amigos[i].aportacion, amigos[i].premio);
				}								
			break;
			//SALIR
			case 0:
				...
				break;
			default:
				printf("Opcion incorrecta, vuelve a probar\n");
			break;
		}
	} while (opcion);
	return 0;
...
int compruebaNum (void) {
	int numero;
	do{
		printf("\nIntroduce el numero que juegas: "); scanf("%d", &numero);
...
int compruebaPremiosMayores (struct Premios premiosMayores[], struct PremioGanado *premioGanado) {	
	if (premioGanado->numConsulta == premiosMayores[0].numero) {
		premioGanado->premio1o = PRIMER_PREMIO /PREMIOSERIE * premioGanado->cantidad;
		strcpy(premioGanado->primerPremio, premiosMayores[0].premio);
		return 1;
	}
	else if (premioGanado->numConsulta == premiosMayores[1].numero) {
		premioGanado->premio2o = SEGUNDO_PREMIO /PREMIOSERIE * premioGanado->cantidad;
		strcpy(premioGanado->segundoPremio, premiosMayores[1].premio);
		return 1;
	}		
	else if (premioGanado->numConsulta == premiosMayores[2].numero){
		premioGanado->premio3o = TERCER_PREMIO /PREMIOSERIE * premioGanado->cantidad;
		strcpy(premioGanado->tercerPremio, premiosMayores[2].premio);
		return 1;
	}	
	else if (premioGanado->numConsulta == premiosMayores[3].numero || premioGanado->numConsulta == premiosMayores[4].numero){
		premioGanado->premio4o = CUARTO_PREMIO /PREMIOSERIE * premioGanado->cantidad;
		...
		for (i = 5; i < PREMIOSMAYORES; i++) {
			if (premioGanado->numConsulta == premiosMayores[i].numero) {
				premioGanado->premio5o = QUINTO_PREMIO /PREMIOSERIE * premioGanado->cantidad;
...
int compruebaPremioConsolacion (struct Premios premiosMayores[], struct PremioGanado *premioGanado) {	
	...
	if(premioGanado->numConsulta == aprox1a || premioGanado->numConsulta == aprox1b) {
		premioGanado->premioAproximacion1o = APROX1O /PREMIOSERIE * premioGanado->cantidad;
		...
		premioGanado->premioAproximacion2o = APROX2O /PREMIOSERIE * premioGanado->cantidad;
		...
		premioGanado->premioAproximacion3o = APROX3O /PREMIOSERIE * premioGanado->cantidad;
	...
	if (premioGanado->numConsulta % 100 == decenas1o) {
		premioGanado->premioDosCifras1o = DECENAS /PREMIOSERIE * premioGanado->cantidad;
		strcpy(premioGanado->dosCifras1o, "Decenas del primer premio");
		hayPremio = 1;
	}
	if (premioGanado->numConsulta % 100 == decenas2o) {
		premioGanado->premioDosCifras2o = DECENAS /PREMIOSERIE * premioGanado->cantidad;
		strcpy(premioGanado->dosCifras2o, "Decenas del segundo premio");
		hayPremio = 1;
	}
	if (premioGanado->numConsulta % 100 == decenas3o) {
		premioGanado->premioDosCifras3o = DECENAS /PREMIOSERIE * premioGanado->cantidad;
	...
	if (premioGanado->numConsulta % 10 == reintegro && premioGanado->numConsulta != premiosMayores[0].numero) {
		premioGanado->premioReintegro1o = REINTEGRO /PREMIOSERIE * premioGanado->cantidad;
...
void imprimePremioConsolacion (struct PremioGanado premioGanado) {
	...
	if(premioGanado.premioAproximacion2o !=0) {
		...
		printf("\nEl numero %d tiene el premio de consolacion '%s'.\nLe corresponden %d euros por serie y has ganado %d euros por tus %d euros jugados\n", premioGanado.numConsulta, premioGanado.aprox2o, APROX2O, premioGanado.premioCentenas1o, premioGanado.cantidad);
	}
...
//FUNCIONES DE LA AMPLIACION

//DEFINIMOS UN MENU
int menu (void) {
	printf("\nUna vez finalizado el sorteo puedes: \n");
	printf("	1. Comprobar uno o varios n%cmeros\n", 163);
	printf("	2. Comprobar premios de un grupo de amigos:\n");
	printf("	0. Salir\n");
	printf("\nEscoge tu opci%cn: ",162); scanf("%d", &opcion);
	return opcion;
}

//FUNCIÓN PARA GESTIONAR EL GRUPO DE AMIGOS Y COMPROBAR LOS PREMIOS
void gestionaGrupo (struct Grupo amigos[]) {
	int aportacion;

	//GENERAMOS LOS NOMBRES
	strcpy(amigos[0].nombre, "Mauro");
	strcpy(amigos[1].nombre,"Toni");
	strcpy(amigos[2].nombre, "Jordi");
	strcpy(amigos[3].nombre, "David");
	strcpy(amigos[4].nombre, "Joel");
	strcpy(amigos[5].nombre, "Iris");
	strcpy(amigos[6].nombre, "Oriol");
	strcpy(amigos[7].nombre, "Pedro");
	strcpy(amigos[8].nombre, "Fer");
	strcpy(amigos[9].nombre, "Adri");
	strcpy(amigos[10].nombre, "Eric");
	strcpy(amigos[11].nombre, "Mar\200al");
	strcpy(amigos[12].nombre, "Rudolph");
	strcpy(amigos[13].nombre, "Xenia");
    strcpy(amigos[14].nombre, "Sergio");    
    strcpy(amigos[15].nombre, "Mou");
    strcpy(amigos[16].nombre, "Oscar");
    strcpy(amigos[17].nombre, "Samuel");
    strcpy(amigos[18].nombre, "Albert");
    strcpy(amigos[19].nombre, "Marti");
	
	//GENERAMOS LA CANTIDAD JUGADA
	for (i = 0; i < MAXAMIGOS; i++) {
		do {
			aportacion = rand() % 61+5;
		} while (aportacion % 5 != 0);
		amigos[i].aportacion = aportacion;		
	}
	
	//NUMERO JUGADO
	amigos[0].numJugado = 25963;
	amigos[1].numJugado = 12486;
	amigos[2].numJugado = 31975;
	amigos[3].numJugado = 52468;
	amigos[4].numJugado = 69443;
	amigos[5].numJugado = 9082;
	amigos[6].numJugado = 17369;
	amigos[7].numJugado = 00302;
	amigos[8].numJugado = 75981;
	amigos[9].numJugado = 87963;
	amigos[10].numJugado = 93647;
	amigos[11].numJugado = 10369;
	amigos[12].numJugado = 55968;
	amigos[13].numJugado = 68354;
	amigos[14].numJugado = 22369;
	amigos[15].numJugado = 257;
	amigos[16].numJugado = 36984;
	amigos[17].numJugado = 95970;
	amigos[18].numJugado = 87342;
	amigos[19].numJugado = 45698;					
}
